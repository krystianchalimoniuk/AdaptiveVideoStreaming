...
public class RtpSocket implements Runnable {
	...
	public void setBitrateController(BitrateAdapter controller){
		mBitrateAdapter = controller;
	}
	...
	public byte[] requestBuffer() throws InterruptedException {
		...
		if (!acquired){
			...
			if (mBitrateAdapter !=null) mBitrateAdapter.onQueueDumped();
			Log.e("TAH","bitrate dropping fifo in request buffer!");
	...
	public void run() {
		...
		try {
			...
			Log.e("TAH","bitrate rtp thread error "+ e.getLocalizedMessage());
			e.printStackTrace();
		}
		if (Thread.currentThread().equals(mThread)){
			mThread = null;
	...
	private void sendTCP() {
		synchronized (mOutputStream) {
			...
			try {
				...
				Log.e("TAH","tcp error "+e.getLocalizedMessage());
			}
		}
	}
